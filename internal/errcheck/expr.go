...
func reduceExprType(pkg *packages.Package, node ast.Expr) types.TypeAndValue {
	...
	case *ast.ParenExpr: return reduceExprType(pkg, stmt.X)
	...
	case *ast.CallExpr:
		...
		case *types.Tuple: return types.TypeAndValue{Type: t.At(0).Type()}
		default: return tav
		}
	case *ast.UnaryExpr: return reduceExprType(pkg, stmt.X)
	case *ast.BinaryExpr:
		t1 := reduceExprType(pkg, stmt.X)
		if t1.Type != nil {
			return t1
		}
		t2 := reduceExprType(pkg, stmt.Y)
		return t2
	default:
